...
<?php
echo "<p>Below is a list of upcoming Academic Advising Center workshops for which you're registered.</p>

<p><strong>Options</strong></p>
<ul>
	<li>See <a href=\"index.php\">our list</a> of our exciting and educational workshops.</li>
	<li>Cancel your registration for a workshop below, if any, by selecting the appropriate button in the \"Cancel\" column and then clicking the \"Submit\" button. You will receive a cancellation email confirmation to your GatorLink email account.</li>
	<li>Save an ical file to your personal calendar by clicking the accompanying iCal icon (<img src=\"/images/calendar.gif\" />) in the table below and saving to your calendar.</li>
</ul>";
...
echo "
  <script type=\"text/javascript\">
    alert(\"Please click the button of the workshop you want to cancel.\");
    history.back();
  </script>";
}

// begin remove student registration from table
elseif (isset($_POST['submit']) AND !empty($_POST['id'])) {
$glid=$_SERVER['HTTP_UFSHIB_GLID'];
$id=$_POST['id'];
$team=$_POST['team'];
...
$sql="DELETE FROM acadvise.wkshp_reg where glid='$glid' and id='$id';";
$result = mysqli_query($mysqli,$sql);
print "<center><h3>Thank you for cancelling your registration.</h3>
<p>Go to the <a href=\"workshoplist.php\">workshop page</a> to see your workshops.</p>
</center>";
// end remove student registration in table




//query the title of the workshop and send it in email
require("/h/cnswww-www.advising/www.advising.ufl.edu/cgi-bin/newlogin.cgi");
...
//send the cancellation notice via email
$to = $mail;
$subject = "You cancelled attendance at an AAC workshop";
$body = "You recently canceled attendance at the \"$title2\" workshop at the UF Academic Advising Center.\n\nIf you had added this event to a personal calendar, then please remember to remove it.\n\nTo see more available workshops, please sign in to https://www.advising.ufl.edu/workshops with your GatorLink.\n\nThank you!\n\n-Academic Advising Center\r
College of Liberal Arts and Sciences\r
University of Florida\r
100 Fletcher Drive - P.O. Box 112015\r
Gainesville, FL  32611-2015\r
Phone:  352-392-1521\r
Fax: 352-392-2905";
$headers = "From: advisor@advising.ufl.edu\r\n" .
   ...
   if (mail($to, $subject, $body, $headers, "-f please-do-not-reply@ufl.edu")) {
	echo("<center><p>Your registration has been cancelled.</p>
</center>");
	} else {
	echo("<center><p>Message delivery failed...</p></center>");
	}
//end cancellation notice via email



}
else {
//show the workshops for which user has registered
echo "<form onsubmit='return validate_form ( ): onSubmit=stamp()' id='sm-form' name='cancel' action='workshoplist.php' method='POST'>";
$i=0;
echo "
<center><h3>Your Workshops</h3>
</center>
<p><strong>If you need to cancel your attendance, please be considerate and cancel 24 hours prior to the workshop!</strong></p>
...
<tr class=\"toprow\">
  ...
  <th>iCal File</th>
  <th>Cancel Your Registration</th>
  </tr>";
require("/h/cnswww-www.advising/www.advising.ufl.edu/cgi-bin/newlogin.cgi");

$sql1="SELECT wkshp_reg.ID as id, wkshp.title as title, wkshp.location as location, wkshp.date as date, DATE_FORMAT(wkshp.timestart, '%h:%i %p') as timestart, DATE_FORMAT(wkshp.timeend, '%h:%i %p') as timeend, wkshp.description, DATE_FORMAT(wkshp.date, '%M %d, %Y') as calyear, DATE_FORMAT(wkshp.timestart, '%H%i%s') as calstart, DATE_FORMAT(wkshp.timeend, '%H%i%s') as calend, wkshp.team as team FROM acadvise.wkshp_reg INNER JOIN acadvise.wkshp ON acadvise.wkshp_reg.ID = acadvise.wkshp.ID where CURDATE() <= date and wkshp_reg.glid='$glid' order by date,timestart,timeend;";
$result1 = mysqli_query($mysqli,$sql1);

$num_rows1 = mysqli_num_rows($result1);

if (!$num_rows1){
echo "<tr><td colspan=\"7\" ALIGN=\"center\"><strong><br />You are not registered for any upcoming workshops.<br /><br /><a href=\"index.php\">See our workshop list to register.</a><br /><br /></strong></td></tr></table>";
}
else {
...
<tr>
    <td>
 	<a href = \"javascript:void(0)\" onclick = \"document.getElementById('light$i').style.display='block';document.getElementById('fade$i').style.display='block'\">{$title1}</a>
	...
	<td>
    {$calyear},<br />{$timestart1} - {$timeend1},<br />{$location1}
    </td>
	
	<td><span title=\"Add to your calendar\"><a href=\"ical/{$id1}.ics\" alt=\"Add to your calendar\"><img src=\"/images/calendar.gif\" /></a></span></td>
	<td>
...
<input type='submit' onsubmit='return validate_form ( );' value='Submit' name='submit' /></form>";
}
}
?>
...
<symbol id="college-logo" viewBox="0 0 2760 200">
